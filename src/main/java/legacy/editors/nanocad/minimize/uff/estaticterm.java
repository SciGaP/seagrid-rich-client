package legacy.editors.nanocad.minimize.uff;

import java.util.Vector;

import legacy.editors.nanocad.*;

public  class estaticterm extends term
{
	...
	// A bunch of dot products- left here because they looked unobtrusive.	
	...
	if(cddotcd <= 0) cddotcd = uffMinimizeAlgorythm.TINY;
	if(uvdotuv <= 0) uvdotuv = uffMinimizeAlgorythm.TINY;
	...
	dedx = firstCharge * secondCharge / (e0 * r*r*r);
	dedthi = firstCharge * secondCharge / (e0 * r*r*r) * (-3 * thetaj);
	dedthj = firstCharge * secondCharge / (e0 * r*r*r) * (-3 * thetai);
    ...
    {
	System.out.println("Creating estaticterm");
	atom tempa;
	int temp;
	myGroup = myAtoms[0].getGroup();
	if(myGroup.needsToGetTypes()) myGroup.setTypes();
	int atom1 = myAtoms[0].getUFFTypeNum();
	int atom2 = myAtoms[1].getUFFTypeNum();
	int atom3 = myAtoms[2].getUFFTypeNum();
	int atom4 = myAtoms[3].getUFFTypeNum();
